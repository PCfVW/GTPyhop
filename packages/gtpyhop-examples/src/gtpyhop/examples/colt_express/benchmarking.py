#!/usr/bin/env python3
"""
Benchmarking Script for Colt Express Examples

Benchmarks colt_express sub-folder domains. Imports shared infrastructure
from ipc-2020-total-order but runs in the colt_express context, which uses
task-list-based scenarios rather than goal-dict scenarios.

Sub-folder naming `sN_<topic>` matches the build-order milestones; numbering
is by build order, not by depth of pattern. See README.md for the
trunk_thumper sub-folder each colt_express folder corresponds to.
...
Examples:
    ...
    python benchmarking.py s2_recursive_round --strategy iterative_dfs_backtracking
...
# Add current directory to path first (for colt_express sub-domains)
...
class ColtExpressBenchmark(PlannerBenchmark):
    """
    Extended benchmark for colt_express domains that use task lists
    instead of goal dictionaries (same pattern as TrunkThumperBenchmark).
    """

    def run_single(self, problem_name, state, tasks, benchmarking_verbose=0, multiple_problems=False):
        """
        Execute a single colt_express planning problem with task-based goals.
        ...
        try:
            with self.track_resources() as tracker:
                if self.use_sessions:
                    ...
                    strategy = getattr(self, '_strategy', 'iterative_greedy')
                    with PlannerSession(domain=self.domain, verbose=session_verbose,
                                        strategy=strategy) as session:
...
def list_available_domains():
    """List available domains in the colt_express directory."""
    domains = []
...
def validate_domain_package(domain_name):
    """Validate that the specified domain exists in colt_express."""
    if not domain_name:
        return False
...
def main():
    """Main entry point for colt_express benchmarking."""
    parser = create_argument_parser()
    parser.add_argument('--strategy',
                        choices=['iterative_greedy', 'recursive_dfs', 'iterative_dfs_backtracking'],
                        default='iterative_greedy',
                        help='Planning strategy (default: iterative_greedy). '
                             'iterative_dfs_backtracking recommended for full coverage '
                             '(s2 recursive deck, s3 negative-control scenario, '
                             's4 priority-method ladder).')
    ...
    if args.list_domains:
        ...
        print("Available colt_express domains:")
    ...
    strategy = getattr(args, 'strategy', None) or "iterative_greedy"

    benchmark = ColtExpressBenchmark(
        domain=domain_obj if use_sessions else domain_module,
        verbose=True, use_sessions=use_sessions
    )
    benchmark._strategy = strategy
    ...
    print(f"\nRunning benchmarks for {domain_name} using {planning_mode} "
          f"planning (strategy: {strategy})...")
