#!/usr/bin/env python3
"""
Benchmarking Script for Poetry Examples

This script benchmarks poetry domains in the current directory.
...
# Add current directory to path first (for poetry domains)
...
class PoetryBenchmark(PlannerBenchmark):
    """
    Extended benchmark for poetry domains that use task lists
    instead of goal dictionaries.
    """

    def run_single(self, problem_name, state, tasks, benchmarking_verbose=0, multiple_problems=False):
        """
        Execute a single poetry planning problem with task-based goals.
...
def list_available_domains():
    """List available domains in the poetry directory."""
    domains = []
...
def validate_domain_package(domain_name):
    """Validate that the specified domain exists in poetry."""
    if not domain_name:
        return False
...
def main():
    """Main entry point for poetry benchmarking."""
    ...
    parser.add_argument('--strategy',
                        ...
                        help='Planning strategy (default: iterative_greedy). '
                             'Use recursive_dfs or iterative_dfs_backtracking for domains '
                             'that require backtracking (backtracking_poetry, replanning_poetry).')
    ...
    if args.list_domains:
        ...
        print("Available poetry domains:")
    ...
    print(f"\nRunning benchmarks for {domain_name} using {planning_mode} planning (strategy: {strategy})...")
