"""
Trunk Thumper Baseline - GTPyhop

Implements the baseline Trunk Thumper domain from Section 12.3 of Troy
Humphreys' "Exploring HTN Planners through Example" (Game AI Pro, 2015).
The root task BeTrunkThumper has two methods: attack a visible enemy, else
patrol bridges. No recursion, no expected effects, no priority subtleties -
the simplest version of the troll.

Key features:
  - 5 actions, 1 task name (m_be_trunk_thumper) with 2 methods
  - 2 scenarios demonstrating attack and patrol behaviors
  - Establishes the action/method naming conventions used across the
    trunk_thumper collection

Reference: Section 12.3 of [Humphreys 15], pp. 154-155.
