#!/usr/bin/env python3
"""
Benchmarking Script for Trunk Thumper Examples

Benchmarks Trunk Thumper sub-folder domains. Imports shared infrastructure
from ipc-2020-total-order but runs in the trunk_thumper context, which uses
task-list-based scenarios rather than goal-dict scenarios.

Sub-folder naming `sNN_<topic>` matches Section 12.NN of Troy Humphreys'
chapter "Exploring HTN Planners through Example" (Game AI Pro, 2015).
...
Examples:
    ...
    python benchmarking.py s07_expected_effects_chase --strategy iterative_dfs_backtracking
...
# Add current directory to path first (for trunk_thumper sub-domains)
...
class TrunkThumperBenchmark(PlannerBenchmark):
    """
    Extended benchmark for trunk_thumper domains that use task lists
    instead of goal dictionaries (same pattern as PoetryBenchmark).
    """

    def run_single(self, problem_name, state, tasks, benchmarking_verbose=0, multiple_problems=False):
        """
        Execute a single trunk_thumper planning problem with task-based goals.
...
def list_available_domains():
    """List available domains in the trunk_thumper directory."""
    domains = []
...
def validate_domain_package(domain_name):
    """Validate that the specified domain exists in trunk_thumper."""
    if not domain_name:
        return False
...
def main():
    """Main entry point for trunk_thumper benchmarking."""
    ...
    parser.add_argument('--strategy',
                        ...
                        help='Planning strategy (default: iterative_greedy). '
                             ...
                             '(s06 recursion, s07 negative-control scenario, s08 priority methods).')
    ...
    if args.list_domains:
        ...
        print("Available trunk_thumper domains:")
    ...
    benchmark = TrunkThumperBenchmark(
